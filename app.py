...
with tab3:
    ...
    if len(st.session_state.responses) > 0:
        ...
        if st.button("📥 PDF 리포트 다운로드"):
            # HTML 리포트 생성
            html_report = "<html><head><meta charset='UTF-8'>"
            html_report += "<style>body{font-family:'Malgun Gothic',sans-serif;margin:20px;}"
            html_report += "h1{color:#2E86AB;text-align:center;}h2{color:#A23B72;margin-top:30px;}"
            html_report += "table{border-collapse:collapse;width:100%;margin:20px 0;}"
            html_report += "th,td{border:1px solid #ddd;padding:8px;text-align:center;}"
            html_report += "th{background-color:#f2f2f2;}.stat{background-color:#e8f4f8;padding:15px;margin:10px 0;border-radius:5px;}</style></head><body>"
            
            html_report += "<h1>형성평가 결과 리포트</h1>"
            html_report += "<div class='stat'>"
            html_report += f"<p><strong>학급:</strong> {class_name}</p>"
            html_report += f"<p><strong>과목:</strong> {subject}</p>"
            html_report += f"<p><strong>일시:</strong> {datetime.now().strftime('%Y년 %m월 %d일 %H시 %M분')}</p>"
            html_report += f"<p><strong>참여 학생:</strong> {len(df)}명</p>"
            html_report += f"<p><strong>평균 이해도:</strong> {df['understanding'].mean():.1f}점 / 5점</p>"
            html_report += "</div>"
            
            html_report += "<h2>학습 목표별 달성률</h2>"
            html_report += "<table><tr><th>학습 목표</th><th>달성률</th></tr>"
            ...
            for i, goal in enumerate(valid_goals):
                ...
                if goal_col in df.columns:
                    achievement_rate = df[goal_col].sum() / len(df) * 100
                    html_report += f"<tr><td>{goal}</td><td>{achievement_rate:.1f}%</td></tr>"
            
            html_report += "</table>"
            
            html_report += "<h2>개인별 현황</h2>"
            html_report += "<table><tr><th>번호</th><th>이해도</th><th>달성 목표</th><th>달성률</th><th>도움 필요</th></tr>"
            
            for _, row in display_df.iterrows():
                html_report += f"<tr><td>{int(row['번호'])}</td>"
                html_report += f"<td>{int(row['이해도'])}</td>"
                html_report += f"<td>{int(row['달성 목표'])}/{int(row['전체 목표'])}</td>"
                html_report += f"<td>{int(row['달성률'])}%</td>"
                html_report += f"<td>{row['도움 필요']}</td></tr>"
